  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "color" ) )
        ...
        throwParseException( "Expected `color', `Kd', `Ka' , 'sign' or }." );
    }
  return new LambertianMaterial( color, Kd, Ka,sign );
}
...
//         else if ( peek( "center1" ) )
//           center1 = parsePoint();
//         else if ( peek( "center2" ) )
//           center2 = parsePoint();
...
//           break;
//         else
//           throwParseException( "Expected `material', `center1', `center2', `radius1','radius2' or }." );
//       }
//         // new Sphere sphere1( material, center1, radius1 );

//           Line->addObject(new Sphere( material, center1, radius1 ) );
//           Line->addObject(new Sphere( material, center2, radius2 ) );
//           Line->addObject(new Cylinder( material, center1, center2, radius1 ) );
//     return Line;
// }
  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "material" ) )
        ...
        throwParseException( "Expected `material', `center1', `center2', `radius' or }." );
    }
  return new Cylinder( material, center1, center2, radius );
}


Object *Parser::parseConeObject()
{
  Material *material = default_material;
  Point center1( 0.0, 0.0, 0.0 );
  Point center2( 0.0, 0.0, 0.0 );
  ...
  return new Cone( material, center1, center2, radius1,radius2);
}
  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "material" ) )
        ...
        throwParseException( "Expected `material', `p1', `p2', `pr1','pr2' or }." );
    }
  return new Line( material, p1, p2, pr1,pr2);
}
...
Parser::Parser(
  ...
  : input( input ),
    line_number( 1 ),
    column_number( 0 ),
    default_material( new LambertianMaterial( Color( 1.0, 1.0, 1.0 ), 0.6, 0.3,0 ))
{
  readNextToken();
}
